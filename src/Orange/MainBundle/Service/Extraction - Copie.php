...
// 				$this->getActiveSheet()->getStyle($b.$y)->applyFromArray($data);
	...
	public function exportSignalisation($arrData){
		...
		$objPHPExcel = new \PHPExcel();
		// Rename sheet
		$objWorkSheet->setTitle("aziz");
		$th = array('Référence Signalisation','Référence de l\'action', 'Instance', 'Périmétre', 'Domaine', 'Type', 'Libellé', 'Description', 'Source', 'Date de signalisation', 'Direction', 'Pôle', 'Département', 'Service','Statut');
		$col = "A";
		$x=1;
		foreach ($th as $value){
			$this->getActiveSheet()->setCellValue($col.$x, $value)->getColumnDimension($col)->setAutoSize(true);
	...
	public function exportCanevas($arrData){
		...
		$th = array('Réference', 'Prénom et Nom du porteur',	'Email porteur','Instance',	'Email(s) contributeur(s)',	'Statut',	'Type',	'Domaine',	'Date de début','Délai initial','Date de clôture','Libellé action',	'Description action','Priorité');
		$col = "A";
		$x=1;
		foreach ($th as $value){
			$this->getActiveSheet()->setCellValue($col.$x, $value)->getColumnDimension($col)->setAutoSize(true);
	...
	public function exportInstance($arrData){
		...
		$objPHPExcel = new \PHPExcel();
		$th = array('Instance', 'Type d\'insatnce', 'Description', 'Animateurs','Domaines','Types');
		$col = "A";
		$x=1;
		foreach ($th as $value){
			if($col == "C"){
