...
class ReportingStructureCommand extends BaseCommand {
	...
	protected function configure() {
		parent::configure ();
		$this->setName ( $this->getName () . ':reporting_structure' )->addOption ( 'projet', 'p', InputOption::VALUE_OPTIONAL )->addOption ( 'espace', 'es', InputOption::VALUE_OPTIONAL )->addOption ( 'bu', 'b', InputOption::VALUE_OPTIONAL )->setDescription ( 'envoi des reporting automatisé' );
	}
	...
	public function execute(InputInterface $input, OutputInterface $output) {
		...
		foreach ( $envois as $envoi ) {
			try {
				...
				$cheminfileException = LogsMailUtils::LogExceptionOnFile ( "ExceptionReportingStructure", $ex );
				$send = $this->getMailer ()->sendLogsMail ( "Exceptions survenu lors du reporting par structures", $this->getTemplating ()->render ( "OrangeMainBundle:Relance:logsMailSend.html.twig", array (
						'libelle' => " exceptions survenu lors du reporting par structure" 
				) ), $chemin );
			}
		}
		if (! empty ( $chemin )) {
