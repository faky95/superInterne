...
class ActionRepository extends BaseRepository {
	public function ActionWithStructureNull() {
		return $this->createQueryBuilder('a')->leftJoin('a.instance', 'inst')->leftJoin('a.porteur', 'port')->leftJoin('a.typeAction', 'typ')->andWhere("a.etatCourant NOT LIKE 'ABANDONNEE_ARCHIVEE'")->andWhere("a.etatCourant NOT LIKE 'SOLDEE_ARCHIVEE'")->andWhere("a.structure IS NULL")->getQuery ()->getResult ();
	}
	...
	public function nouvelleAction($bu, $espace, $projet) {
		$date = new \DateTime ();
		return $this->createQueryBuilder('a')->innerJoin('a.porteur', 'u')->innerJoin('a.instance', 'i')->innerJoin('a.domaine', 'd')->andWhere("a.etatReel LIKE 'ACTION_NOUVELLE'")->orderBy('a.id', 'ASC')->getQuery ()->execute ();
	}
	public function userToAlertDepassement($bu, $espace, $projet) {
		$date = new \DateTime('@' . strtotime('+3 days'));
		$queryBuilder = $this->createQueryBuilder('a')->innerJoin('a.porteur', 'u')->innerJoin('a.instance', 'i')->innerJoin('a.domaine', 'd')->where('a.dateInitial <= :date')->andWhere("a.etatCourant LIKE 'ACTION_NON_ECHUE'")->orderBy('a.id', 'ASC')->addOrderBy('a.dateAction', 'DESC')->setParameter('date', $date)->getQuery ()->execute ();
		return $queryBuilder;
	}
	...
	public function getStatsByInstance2($role, $criteria) {
		...
		$queryBuilder->leftJoin($alias . '.actionCyclique', 'acl1')->leftJoin('acl1.tache', 't1')->leftJoin($alias . '.instance', 'i')->innerJoin($alias . '.porteur', 'u')->innerJoin($alias . '.structure', 's')->andWhere($alias . ".etatCourant NOT LIKE 'ABANDONNEE_ARCHIVEE' AND " . $alias . ".etatCourant NOT LIKE 'SOLDEE_ARCHIVEE'")->select($alias . '.id');
	...
	public function getStatsByInstance($role, $criteria) {
		...
		$alias = $queryBuilder->getRootAlias ();
		$queryBuilder->leftJoin($alias . '.actionCyclique', 'acl1')->leftJoin('acl1.tache', 't1')->leftJoin($alias . '.instance', 'i')->innerJoin($alias . '.porteur', 'u')->innerJoin($alias . '.structure', 's')->select('count(' . $alias . '.id) total ,' . $alias . '.etatCourant action_etat, t1.etatCourant tache_etat, i.id, i.libelle')->groupBy('i.id')->addGroupBy($alias . '.etatCourant')->addGroupBy('t1.etatCourant');
		if ($role === Utilisateur::ROLE_MANAGER)
			$queryBuilder->andWhere('u!=:me')->setParameter('me', $this->_user);
		$this->filtres($queryBuilder, $criteria, $alias);
		return $queryBuilder;
	...
	public function getStatsGeneralByRole($role) {
		...
		$alias = $queryBuilder->getRootAlias ();
		$queryBuilder->leftJoin($alias . '.actionCyclique', 'acl1')->leftJoin('acl1.tache', 't1')->leftJoin($alias . '.instance', 'i')->select('count(' . $alias . '.id) total ,' . $alias . '.etatCourant action_etat, t1.etatCourant tache_etat')->groupBy($alias . '.etatCourant')->addGroupBy('t1.etatCourant');
		$data = $queryBuilder->getQuery ()->getArrayResult ();
		$data = $this->combineTacheAndAction($data);
		return $data;
	}
